...
func Test_PackageInstall_Dependencies_Installed_Successfully(t *testing.T) {
	...
	cleanUp := func() {
		...
		kubectl.RunWithOpts([]string{"delete", "packageinstalls/" + getPackageInstallByPackageName(&kubectl, "pkg.child.carvel.dev")[0]}, e2e.RunOpts{AllowError: true})
		kubectl.RunWithOpts([]string{"delete", "packageinstalls/" + getPackageInstallByPackageName(&kubectl, "pkg.child2.carvel.dev")[0]}, e2e.RunOpts{AllowError: true})
...
`)
	...
	logger.Section("Create PackageInstall and verify dependencies installed", func() {
		...
		kubectl.Run([]string{"wait", "--for=condition=ReconcileSucceeded", "packageinstalls/" + getPackageInstallByPackageName(&kubectl, "pkg.child.carvel.dev")[0], "--timeout", "1m"})
		kubectl.Run([]string{"wait", "--for=condition=ReconcileSucceeded", "packageinstalls/" + getPackageInstallByPackageName(&kubectl, "pkg.child2.carvel.dev")[0], "--timeout", "1m"})
	})

}

func Test_PackageInstall_Dependencies_Installed_Successfully_With_Override(t *testing.T) {
	env := e2e.BuildEnv(t)
	logger := e2e.Logger{}
	kubectl := e2e.Kubectl{T: t, Namespace: env.Namespace, L: logger}
...
kind: Package
metadata:
  name: pkg.child.carvel.dev.2.0.0
spec:
  refName: pkg.child.carvel.dev
  version: 2.0.0
  template:
...
kind: Package
metadata:
  name: pkg.child.carvel.dev.3.0.0
spec:
  refName: pkg.child.carvel.dev
  version: 3.0.0
  template:
...
`)
	...
	logger.Section("Create PackageInstall and verify dependencies installed", func() {
		kubectl.RunWithOpts([]string{"apply", "-f", "-"}, e2e.RunOpts{StdinReader: strings.NewReader(packageInstallYaml)})
		kubectl.Run([]string{"wait", "--for=condition=ReconcileSucceeded", "packageinstalls/" + "parent-pkgi", "--timeout", "1m"})
		childPkgi := getPackageInstallByPackageName(&kubectl, "pkg.child.carvel.dev")
		require.Equal(t, childPkgi[2], "3.0.0")
		kubectl.Run([]string{"wait", "--for=condition=ReconcileSucceeded", "packageinstalls/" + childPkgi[0], "--timeout", "1m"})
		kubectl.Run([]string{"wait", "--for=condition=ReconcileSucceeded", "packageinstalls/" + getPackageInstallByPackageName(&kubectl, "pkg.child2.carvel.dev")[0], "--timeout", "1m"})
	})
...
func getPackageInstallByPackageName(kubectl *e2e.Kubectl, packageName string) []string {
	...
	for _, line := range lines {
		...
		if len(fields) >= 3 && fields[1] == packageName {
